...
from flask import render_template,session,redirect,url_for,flash,make_response,abort,request,current_app
...
def followers(username):
	...
	if user is None:
		...
		return redirect(url_for('.index'))
	page=request.args.get("page",1,type=int)
	pagination=user.followers.paginate(page,per_page=current_app.config["FLASKY_FOLLOWS_PER_PAGE"],error_out=False)
	follows=[{"user":item.follower,"timestamp":item.timestamp}
			for item in pagination.items] 
	return render_template("followers.html",user=user,title=u"关注者",endpoint=".followers",pagination=pagination,follows=follows)
...
def followed_by(username):
	...
	if user is None:
		...
		return redirect(url_for('.index'))
	page=request.args.get("page",1,type=int)
	pagination=user.followed.paginate(page,per_page=current_app.config["FLASKY_FOLLOWS_PER_PAGE"],error_out=False)
	follows=[{"user":item.followed,"timestamp":item.timestamp}
			for item in pagination.items] 
	return render_template("followers.html",user=user,title=u"被关注者",endpoint=".followed_by",pagination=pagination,follows=follows)
...
@main.route("/miderate/disable/<int:id>")
@login_required
@permission_required(Permission.MODERATE_COMMENTS)
