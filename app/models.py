...
class User(UserMixin,db.Model):
	...
	def __init__(self,**kwargs):
		...
		if self.role is None:
			if self.email==current_app.config["BLOG_MAIL_ADMIN"]:
	...
	def generate_confirmation_token(self,expiration=3600):#生成序列号
		s=serializer(current_app.config["SECRET_KEY"],expiration )
		return s.dumps({"confirm":self.id})

	def confirm(self,token,expiration=3600):#验证序列号
		s=serializer(current_app.config["SECRET_KEY"],expiration )
		try:
			data=s.loads(token)
		except:
...
class Post(db.Model):
	...
	def from_json(json_post):
		...
		if body is None or body =="":
			raise ValidationError(u"博文没有主体")
	...
	def to_json(self):
		json_post={
				  ...
				  "commnets":url_for("api.get_post_comments",id=self.id,_external=True),
...
class Comment(db.Model):
	...
	def from_json(json_comment):
		...
		if body is None or body =="":
			raise ValidationError(u"评论内容为空")
